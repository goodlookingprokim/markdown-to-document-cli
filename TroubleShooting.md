...
Windows에서 처음 시작하시나요? 상세 설치 가이드를 참조하세요.

> **[WINDOWS_QUICKSTART.md](./WINDOWS_QUICKSTART.md)** - Windows 완전 초보자를 위한 상세 설치 가이드 (Node.js, Pandoc, Python, WeasyPrint 설치)
...
### 💡 Windows 터미널 붙여넣기 방법 (초보자 필독!)

Windows CMD나 PowerShell에서 명령어를 붙여넣을 때 **Ctrl+V가 작동하지 않는 경우**가 있습니다. 아래 방법들을 시도해보세요:

#### **방법 1: 마우스 오른쪽 클릭 (가장 확실!)**
```
1. 명령어 복사 (Ctrl+C)
2. CMD/PowerShell 창에서 마우스 오른쪽 클릭
3. 자동으로 붙여넣기 완료!
```

#### **방법 2: Ctrl+Shift+V**
```
일부 최신 Windows 터미널에서는 Ctrl+Shift+V로 붙여넣기 가능
```

#### **방법 3: 터미널 설정에서 Ctrl+V 활성화**
```
1. CMD 창 제목 표시줄 우클릭 → "속성" 클릭
2. "옵션" 탭에서 "빠른 편집 모드" 체크 ☑️
3. "Ctrl 키 바로 가기 사용" 체크 ☑️
4. 확인 클릭 → 이제 Ctrl+V 사용 가능!
```

#### **방법 4: Windows Terminal 사용 (권장)**
```
1. Microsoft Store에서 "Windows Terminal" 설치
2. Windows Terminal은 Ctrl+V가 기본으로 작동
3. 탭 기능, 더 나은 UI 제공
```

#### **붙여넣기 확인 팁**
- 붙여넣기 후 **명령어가 보이는지 확인** 후 Enter
- 긴 명령어는 **한 줄씩** 붙여넣기
- 특수문자(`&`, `|`, `>` 등)가 있으면 **따옴표로 감싸기**
...
## ✅ [v1.5.7에서 해결됨] Mac vs Windows PDF 결과물 차이

> **v1.5.7부터 이 문제가 해결되었습니다.** Windows에서 WeasyPrint 없이 PDF 변환 시 에러가 발생하며, 설치 안내가 표시됩니다.

### 🔄 v1.5.7 변경 사항

**이전 동작 (v1.5.6 이하)**:
- Windows에서 WeasyPrint가 없으면 XeLaTeX 자동 사용
- 경고만 표시하고 변환 진행 → HTML 태그 노출 문제

**새로운 동작 (v1.5.7 이상)**:
- Windows에서 WeasyPrint **필수**
- WeasyPrint 없이 PDF 변환 시 **에러 발생** + 설치 안내
- 명시적으로 `--pdf-engine=xelatex` 지정 시에만 LaTeX 사용 가능 (경고 표시)
...
# 2. 정상 변환 - Mac과 100% 동일한 결과
npx markdown-to-document-cli@latest document.md --format pdf
```

이제 Mac과 **동일한 고품질 PDF**를 받을 수 있습니다.

### 📋 이전 문제 요약

| 항목 | Mac (WeasyPrint) | Windows (MiKTeX) |
|------|------------------|------------------|
| **PDF 엔진** | WeasyPrint | MiKTeX-dvipdfmx |
| **HTML 처리** | ✅ 완벽 렌더링 | ❌ HTML 태그 노출 |
| **페이지 수** | 더 많음 | 더 적음 |
...
1. **PDF 엔진의 근본적 차이**
   ...
   - **MiKTeX**: HTML → LaTeX → PDF 변환 (HTML 태그 노출 발생)

2. **Cover 시스템과의 호환성**
   - 표지가 HTML/CSS (Flexbox, Gradient)로 생성됨
   - LaTeX는 이런 현대 CSS를 지원하지 않음 → 렌더링 실패

### 💡 LaTeX 강제 사용 (권장하지 않음)

특수한 상황에서 LaTeX를 사용해야 한다면:

```bash
# XeLaTeX 강제 사용 (경고 표시됨)
m2d document.md --pdf-engine xelatex

# PDFLaTeX 강제 사용 (경고 표시됨)
m2d document.md --pdf-engine pdflatex
```

> ⚠️ **주의**: LaTeX 엔진 사용 시 HTML 태그 노출, 레이아웃 깨짐이 발생할 수 있습니다.
