...
## 1) Interactive Mode 워크플로우 (v1.2.3 개선)

v1.2.3부터 **간소화된 3단계 워크플로우**가 적용되었습니다:

| Step | 내용 | 설명 |
|------|------|------|
| **Step 1** | 📄 파일 선택 | 마크다운 파일 경로 입력 |
| **Step 2** | 🚀 모드 선택 | 변환 모드 선택 (빠른/상세/전처리만) |
...
> 💡 **v1.2.3 이후**: 위 항목이 감지되면 "빠른 변환" 모드에서도 자동으로 전처리가 적용됩니다.
...
- Obsidian 노트 기반 + 품질 중요 → **전처리 후 변환(A)**
- 표준 Markdown + 빠른 결과 확인 → **바로 변환(B)**
